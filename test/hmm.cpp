...
TEST(HMMTest, LikelihoodNormal) {
  ...
  dist3.fix_p(true);
  dist4.fix_p(true);
  auto dist = make_hmm(dist1, dist2);
  auto dist_a = make_hmm(dist3, dist4);
  ...
  auto new_dist = make_hmm(Laplace<double>(-0.1, 1),
...
TEST(HMMTest, LikelihoodLaplace) {
  boost::random::mt19937 rng;
  const unsigned int n_samples = 100;
  ...
  auto new_dist = make_hmm(Normal<double>(-0.1, 1),
      Normal<double>(0.1, 1));
...
TEST(HMMTest, LikelihoodAsymmetricNormal) {
  boost::random::mt19937 rng;
  const unsigned int n_samples = 100;
  ...
  dist1.fix_p(true);
  dist2.fix_p(true);
  auto dist_a = make_hmm(dist1, dist2);
  Array<double> samples;
  dist.sample(samples, n_samples, rng);
...
TEST(HMMTest, LikelihoodAsymmetricLaplace) {
  boost::random::mt19937 rng;
  const unsigned int n_samples = 100;
